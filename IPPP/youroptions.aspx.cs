...
namespace APT2012.IPPP
{
    public partial class youroptions : System.Web.UI.Page
    {
        ...
        protected void btnOptA_Click(object sender, EventArgs e)
        {
            try
            {
                string option = Utility.GetAppSettingValue("IPPP_Options_Email_A");
                SendMail(option);
        ...
        protected void btnOptB_Click(object sender, EventArgs e)
        {
            try
            {
                string option = Utility.GetAppSettingValue("IPPP_Options_Email_B");
                SendMail(option);
                hdnMsg.Value = Utility.GetAppSettingValue("EmailSuccess");
            }
            catch (Exception ex)
        ...
        protected void btnOptD_Click(object sender, EventArgs e)
        {
            try
            {
                string option = Utility.GetAppSettingValue("IPPP_Options_Email_D");
                SendMail(option);
                hdnMsg.Value = Utility.GetAppSettingValue("EmailSuccess");
            }
            catch (Exception ex)
        ...
        protected void btnOptC_Click(object sender, EventArgs e)
        {
            try
            {
                string option = Utility.GetAppSettingValue("IPPP_Options_Email_C");
                SendMail(option);
                hdnMsg.Value = Utility.GetAppSettingValue("EmailSuccess");
            }
            catch (Exception ex)
